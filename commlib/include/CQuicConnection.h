...
typedef CommLibBoolean(COMMLIB_API* QuicListenerNewConnectionCallback)(
    QuicConnection* Connection, void* Context);
...
typedef void(COMMLIB_API* QuicConnectionDatagramDataCallback)(
    void* Context, const QuicDataBuffer* DataBuffer);
typedef void(COMMLIB_API* QuicConnectionStreamDataCallback)(
    void* Context, uint32_t StreamId, const QuicDataBuffer* DataBuffers,
    uint32_t DataBuffersSize);
...
CommLibStatus COMMLIB_API QC_CreateListener(const QuicRegistration* Registration,
                                uint16_t Port, const uint8_t* Alpn,
                                uint16_t AlpnLength, const uint8_t* PfxBuffer,
                                uint32_t PfxSize, const char* PfxPassword,
                                uint32_t NumStreams,
...
// Listener must be stopped, and QuicListenerStopped event must have been
// received in order to call this API. Additionally, all connections received
// from the listener must have been freed.
